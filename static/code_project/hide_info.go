...
func ReviewInfo(pixel_array []byte) []byte {
	// TODO Your code here
}
...
func ReviewProcedure(src_img_path string, data_path string) {
	fmt.Printf("Review hidden info from %v, then write it to %v\n",
		src_img_path, data_path)
	_, _, pixel_array := GetPartsOfBmp(src_img_path)
	info := ReviewInfo(pixel_array)
...
func _print_usage() {
	fmt.Fprintf(os.Stderr, "* hide args: hide <src_img_path> <hide_file_path> "+
		"<dest_img_path>\n")
	fmt.Fprintf(os.Stderr, "* reivew args: review <img_path> <data_file>\n")
}
		...
		case "review":
			{
				if len(os.Args) < 4 {
					...
					ReviewProcedure(os.Args[2], os.Args[3])
				}
			}
		default:
			_print_usage()
